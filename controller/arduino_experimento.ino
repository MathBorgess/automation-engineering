 ...
 * - Envia altura da bolinha medida via Serial
 */
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (comando.startsWith("FAN:")) {
      ...
      if (valor >= 0 && valor <= 100) {
        ...
        // Debug: confirmar aplicação
        Serial.print("FAN aplicado: ");
        Serial.print(velocidade_fan);
        Serial.print("% (PWM: ");
        Serial.print(pwm_value);
        Serial.println(")");
      } else {
        Serial.print("ERRO: Valor inválido: ");
  ...
  if (tempo_atual - ultima_leitura >= INTERVALO_LEITURA) {
    ...
    // Medir distância do sensor até a bolinha
    float distancia = ler_sensor_ultrasonico();
    
    // Calcular altura da bolinha
    // Assumindo que o sensor está no topo do tubo (altura_tubo + altura_sensor)
    // Altura da bolinha = (altura_total) - distancia - raio_bolinha
    // Para simplificar, vamos enviar a distância medida e o Python calcula a altura
    // Ou podemos calcular aqui se soubermos as dimensões do tubo
    
    // Enviar distância via Serial
    // Formato: "DIST:25.5" (distância do sensor até a bolinha em cm)
    Serial.print("DIST:");
